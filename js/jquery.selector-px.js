;(function($){
    ...
    $('body').append('<style>'+
                ...
                '.sel-box .btn{background:#f90;overflow:hidden;}'+
                ...
                '.sel-con{background:white;position:relative;}'+
                '.sel-con .border{height:34px;border:solid 1px #f90;border-width:1px 0;position:absolute;bottom:72px;left:0;right:0;pointer-events:none;}'+
                ...
                '.sel-con .ele{font-size:16px;color:#b2b2b2;height:36px;line-height:36px;overflow:hidden;}'+
    ...
    $.areaSelector = function(o) {
        ...
        // 插件自带
        $(o.evEle).attr('readonly','readonly');

        $(o.evEle).on(o.type, function (){
            $('input, textarea').each(function(){
                this.blur();
            });
            $('.sel-boxs .bg')[0].addEventListener('touchmove', preDef, false);
            $('.sel-boxs .btn')[0].addEventListener('touchmove', preDef, false);
            $('.sel-box .name').text(o.title);
            $('.sel-boxs').show().find('.sel-box').removeClass('fadeInDown').addClass('fadeInUp');
            ...
            var scText1 = "", scText2 = "", scText3 = ""; // 所选值
            // 根据code返回html片段
            function getHtmlByCode(code) {
                var inner = "";
                var obj = ChineseDistricts[code];
                for(var i in obj) {
                    inner += "<div class=\"ele\" sid=\""+i+"\">"+obj[i]+"</div>";
                }
                return inner;
            }
            // 省份渲染
            function updateProv(code) {
                $('.sel-con .scroll').eq(0).html(getHtmlByCode(code));
            }
            // 市渲染
            function updateCity(code) {
                var c = code ? code : $('.sel-con .scroll').eq(0).find('.ele').eq(0).attr("sid");
                $('.sel-con .scroll').eq(1).scrollTop(0);
                $('.sel-con .scroll').eq(1).html(getHtmlByCode(c));
            }
            // 区县渲染
            function updateTown(code) {
                var c = code ? code : $('.sel-con .scroll').eq(1).find('.ele').eq(0).attr("sid");
                $('.sel-con .scroll').eq(2).scrollTop(0);
                $('.sel-con .scroll').eq(2).html(getHtmlByCode(c));
            }

            // 初始化
            (function() {
                var scrollInner = "<div class=\"cell elem\"><div class=\"scroll\"></div></div>";
                // 添加三个选择框
                $('.sel-con .table').html(scrollInner+scrollInner+scrollInner);
                updateProv(86);updateCity();updateTown(); 
                scText1 = $('.sel-con .scroll').eq(0).find('.ele').eq(0).text();
                scText2 = $('.sel-con .scroll').eq(1).find('.ele').eq(0).text();
                scText3 = $('.sel-con .scroll').eq(2).find('.ele').eq(0).text();
            })();
            ...
            $('.sel-con .scroll').eq(0).scroll(function(){
                ...
                $(this).attr('timer',setTimeout(function(){
                    that[0].scrollTop = scNum*36;
                    // 渲染市
                    var code = that.find('.ele').eq(scNum).attr("sid");
                    updateCity(code);
                    updateTown(); // 不加参数，默认渲染第一个市的区县
                },100));
            });
            // 市滚动时
            ...
            // 确认选择
            $('.sel-box .ok').click(function(){
                o.afterAction(scText1, scText2, scText3);
